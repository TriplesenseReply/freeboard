	...
	this.saveDashboard = function()
	{
        ...
        if (q != undefined){
            ...
            for(var i = 0; i < q.length; i++){
                hash = q[i].split('=');
                getParam.push(hash[1]);
                getParam[hash[0]] = hash[1];
            }
        }

        var boardName = prompt('Specify a name to store this board or leave empty to overwrite the current board.');
        ...
        if (boardName != null) {
            $.post(
                "/operations.php",
                {
                    ...
                    board_data: JSON.stringify(self.serialize()),
                    board_file: getParam['board']
                },
